...
\sysname relies on three key mechanisms to empower applications to regain control over scheduling. 
First, \sysname provides the \textbf{Policy Engine} as the developer-facing interface for specifying scheduling policy and intent. 
Applications formulate scheduling policy using a \textit{rank} function, while declaring intent through SLO constraints such as tail latency or deadline.
Second, \sysname uses the \textbf{Synthesis Engine} to aautomatically generate a scheduling implementation from the application's policy and intent specifications.
It analyzes the rank function to determine when rescheduling should occur and maps these triggers to low-level hardware events such as I/O completions, timer expirations, or queue state changes. 
The engine then generates a scheduling handler that recomputes priorities and selects the next task when those events occur.
Finally, \sysname enforces the synthesized implementation through a trusted scheduler, \textbf{MesoCore}.
MesoCore runs inside the \textbf{Shadow Ring} and interacts with low-level mechanisms required for microsecond-scale enforcement, including user interrupts and hardware timers, to prevent malicious or buggy applications from compromising the system's security and stability.
